...
public class TSRSBakedMenger implements IBakedModel {
	...
	public TSRSBakedMenger(IBakedModel original, float x, float y, float z, float scale) {
		this(original, x, y, z, 0, 0, 0, scale, scale, scale);
	}

	public TSRSBakedMenger(IBakedModel original, float x, float y, float z, float rotX, float rotY, float rotZ,float scale) {
		this(original, x, y, z, rotX, rotY, rotZ, scale, scale, scale);
	}

	public TSRSBakedMenger(IBakedModel original, float x, float y, float z, float rotX, float rotY, float rotZ,float scaleX, float scaleY, float scaleZ) {
		this(original, new TRSRTransformation(new Vector3f(x, y, z), null, new Vector3f(scaleX, scaleY, scaleZ),TRSRTransformation.quatFromXYZ(rotX, rotY, rotZ)));
	}

	public TSRSBakedMenger(IBakedModel original, TRSRTransformation transform) {
	...
	/**
	 * Rotates around the Y axis and adjusts culling appropriately. South is
	 * default.
	 */
	public TSRSBakedMenger(IBakedModel original, EnumFacing facing) {
		this.original = original;
		this.override = new TRSROverride(this);
	...
	private static class TRSROverride extends ItemOverrideList {
		...
		private final TSRSBakedMenger model;

		public TRSROverride(TSRSBakedMenger model) {
		...
		public IBakedModel handleItemState(@Nonnull IBakedModel originalModel, ItemStack stack, @Nonnull World world,@Nonnull EntityLivingBase entity) {
			...
			return new TSRSBakedMenger(baked, model.transformation);
		}
	}
